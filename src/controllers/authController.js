...
AuthController.googleAuth = function (req, res) {
    let token = generateAccessToken(req.user.id, req.user.email, 'google');
    res.json({ success: true, token: "jwt " + token })
}

AuthController.facebookAuth = function (req, res) {
    let token = generateAccessToken(req.user.id, req.user.email, 'facebook');
    res.json({ success: true, token: "jwt " + token })
}
...
function generateAccessToken(userId, email, authType) {
    ...
    const token = jwt.sign({ email: email, authType: authType },
        secret, {
            expiresIn: expiresIn,
